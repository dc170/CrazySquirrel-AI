...
#
# Modified by Pablo Gay pablo.gay@udg.edu
#
...
from squirrel_ai import SquirrelAI
...
GAMEOVERTIME = 120   # how long the "game over" text stays on the screen in seconds
...
Enemy Squirrel data structure keys:
    ...
    'id' - unique identifier of this squirrel
...
def main():
    ...
    L_SQUIR_IMG = pygame.image.load('resources/squirrel.png')
...
def runGame():
    ...
    while True: # main game loop
        ...
        while len(squirrelObjs) < NUMSQUIRRELS:
            squirrelObjs.append(makeNewSquirrel(camerax, cameray, squirrels_id))
            squirrels_id += 1
        ...
        # call the ia to get the next move (up, down, left, right)
        playerSq = {'x':      playerObj['x'],
                    'y':      playerObj['y'],
                    'size':   playerObj['size'],
                    'health': playerObj['health']
                    }
        otherSq = []
        for oSq in squirrelObjs:
              otherSq.append({'x':      oSq['x'],
                              'y':      oSq['y'],
                              'height': oSq['height'],
                              'width':  oSq['width'],
                              'movex':  oSq['movex'],
        ...
        moveUp, moveDown, moveLeft, moveRight = ai.moveSquirrel(playerSq, otherSq)
        ...
        if not gameOverMode:
            # Calculate Score
            scoreSurf = BASICFONT.render('{} {}'.format(playerObj['size']**2, str(datetime.now()-ts_start)), True, WHITE)
            scoreRect = winSurf2.get_rect()
            scoreRect.center = (HALF_WINWIDTH, 30)
...
def makeNewSquirrel(camerax, cameray, sid):
    sq = {}
